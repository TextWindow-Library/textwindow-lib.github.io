...
Continue Development - 8/9/2019 @10:46 - 
*/
...
TextWindow.KeyEventListener = function (event) {
  console.log("KeyEventListener Triggered with Keycode: " + event.keyCode);
  if (keyType == "read") {
...
TextWindow.KeyEventHandler = function (type, display, functionOut) {
  console.log(TextWindow.KeyEventHandler.caller);
  if (keyInit == false) {
    console.log("EventHandler Error 1");
    return false;
  }
  if (type != "read" && type != "readkey") {
    console.log("EventHandler Error 2");
    return false;
  } else if (display != true && display != false) {
    console.log("EventHandler Error 3");
    return false;
  } else if (eval("typeof " + functionOut) != "function") {
    console.log("EventHandler Error 4");
    return false;
  } else if (keyInit == false) {
    console.log("EventHandler Error 5");
    return false;
  } else {
    keyType = type;
    keyDisplay = display;
...
TextWindow.Read = function (showInDisplay, outputFunction) {
  if (keyInit == false) {
    console.log("KeyInit Error")
    return false;
  }
  if (outputFunction != null && eval("typeof " + outputFunction) != "function") {
    console.log("OutputFunction Error")
    return false;
  } else {
    return TextWindow.KeyEventHandler("read", showInDisplay, outputFunction);
  }
}

TextWindow.ReadKey = function (outputFunction) {
  if (keyInit == false) {
    return false;
  }
  if (outputFunction != null && eval("typeof " + outputFunction) != "function") {
    ...
    return TextWindow.KeyEventHandler("readkey", false, outputFunction);
  }
}
